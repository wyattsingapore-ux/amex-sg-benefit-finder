#!/usr/bin/env python3
"""Refresh today's Eatigo time/discount snapshot and source-provided cuisine labels.

Reads data/merchants.json, fetches each Eatigo branch page over ordinary HTTP,
keeps only Singapore-local future slots for today, and writes data/eatigo_today.json.
The booking source of truth remains Eatigo; the snapshot is for discovery only.
"""
...
def parse_cuisines(html: str) -> list[str]:
    ...
    start = next((i for i, x in enumerate(lines) if x.lower() == "cuisines"), None)
    if start is None:
        return []
    collected: list[str] = []
...
def fetch_one(row: dict, now: datetime) -> dict:
    ...
    result = {
        "id": row.get("id"), "name": row.get("name"), "branch_id": bid,
        "eatigo_url": url, "address": row.get("address"),
        "postal_code": row.get("postal_code"), "lat": row.get("lat"),
        "lng": row.get("lng"), "lc": bool(row.get("lc")),
        "cuisines": [], "slots": [], "best_today": None, "error": None,
    }
    if not url:
...
def main() -> int:
    ...
    ap.add_argument("--limit", type=int, default=0, help="0 means all Eatigo rows")
    ap.add_argument("--workers", type=int, default=8)
    args = ap.parse_args()
    started = time.perf_counter()
    now = datetime.now(SG)
    payload = json.loads(Path(args.merchants).read_text(encoding="utf-8"))
    rows = [m for m in payload.get("merchants", []) if m.get("eatigo") and m.get("eatigo_url")]
    if args.limit > 0:
        rows = rows[: args.limit]
    if not rows:
        raise SystemExit("No Eatigo merchant rows available")
    results: list[dict] = []
    ...
    out = {
        "snapshot": "eatigo-today-v1", "date_sg": now.date().isoformat(),
        "fetched_at": datetime.now(SG).isoformat(),
        "source": "Eatigo branch pages; booking availability must be confirmed on Eatigo",
        "restaurants_attempted": len(results), "restaurants_with_future_slots": len(usable),
        ...
        "request_errors": len(errors), "refresh_seconds": elapsed,
        "restaurants_per_second": round(len(results) / elapsed, 2) if elapsed else None,
        "workers": args.workers,
        "cuisine_types": [{"name": name, "count": count} for name, count in sorted(cuisine_counts.items(), key=lambda kv: (-kv[1], kv[0].lower()))],
        "restaurants": results,
    }
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    summary_keys = ["date_sg","restaurants_attempted","restaurants_with_future_slots","restaurants_mapped_with_future_slots","restaurants_with_cuisine","restaurants_with_50pct_or_better","request_errors","refresh_seconds","restaurants_per_second","workers"]
    print(json.dumps({k: out[k] for k in summary_keys}, indent=2))
    if len(results) >= 200 and len(usable) < int(len(results) * 0.30):
        raise SystemExit(f"Eatigo snapshot produced too few usable slot pages: {len(usable)}/{len(results)}")
    return 0
