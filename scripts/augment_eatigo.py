...
need restaurant/listing membership, an Eatigo link, and a conservative Eatigo+LC
intersection.

The crawler uses ordinary HTTP requests against Eatigo's server-rendered public
Singapore listing pages. It follows same-region category/theme/tag pages to
broaden coverage, so GitHub Actions no longer needs Chromium or Playwright.
"""
...
def qualifier_score(eatigo: dict, merchant: dict) -> float:
    ...
    if not qualifier:
        return 0.0
    hay = " ".join(str(x or "") for x in [merchant.get("brand"), merchant.get("name"), merchant.get("address")])
    q, h = key_text(qualifier), key_text(hay)
    if q and q in h:
        return 1.0
    q_tokens, h_tokens = set(q.split()), set(h.split())
...
def is_region_listing(url: str) -> bool:
    ...
    if u.netloc not in {"eatigo.com", "www.eatigo.com"}:
        return False
    p = u.path.rstrip("/")
    if not p.startswith("/en/regions/27"):
        return False
    if "/branches/" in p:
        return False
...
def parse_listing(html: str, page_url: str) -> tuple[list[dict], list[str]]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if bm:
            ...
            if current is None or len(name) > len(current["name"]):
                restaurants[bid] = {"branch_id": bid, "name": name, "url": full}
        elif is_region_listing(full) and full != canonical_url(page_url):
            listing_urls.add(full)

    return list(restaurants.values()), sorted(listing_urls)
...
def discover_eatigo() -> tuple[list[dict], int]:
    session = requests.Session()
    session.headers.update({"User-Agent": UA, "Accept-Language": "en-SG,en;q=0.9"})

    # Seed several public sort variants plus pages discovered from Eatigo itself.
    seeds = [
        SEARCH_URL,
        SEARCH_URL + '?filter={"sortby":"popular"}',
        SEARCH_URL + '?filter={"sortby":"rating"}',
        SEARCH_URL + '?filter={"sortby":"discount"}',
        "https://eatigo.com/en/regions/27/themes/14764",  # View all restaurants
        "https://eatigo.com/en/regions/27/categories/14764",
        "https://eatigo.com/en/regions/27/themes/14758",  # New on Eatigo
        "https://eatigo.com/en/regions/27/themes/14767",  # Most booked
        "https://eatigo.com/en/regions/27/themes/15814",  # SG makan map
    ]
    ...
    while q and len(seen_pages) < MAX_LISTING_PAGES:
        ...
        try:
            ...
            rows, more = parse_listing(html, url)
        except Exception as exc:
    ...
    print(json.dumps({"eatigo_listing_pages_checked": len(seen_pages), "eatigo_listing_pages_failed": failed_pages, "eatigo_unique_restaurants": len(rows)}, indent=2))
    return rows, len(seen_pages)
...
def main() -> int:
    ...
    eatigo_outlets, pages_checked = discover_eatigo()
    ...
    for e in eatigo_outlets:
        ...
        merchants.append({
            "name": e["name"], "brand": head, "address": approx_address, "postal_code": None,
    ...
    print(json.dumps({"eatigo_list": len(eatigo_outlets), "eatigo_lc": matched, "time_slots_collected": False}, indent=2))
    return 0
